...
def test_reserve_mm_ipc_gpu_memory_uses_multimodal_reservation():
    mm_config = SimpleNamespace()
    worker = _worker_with_mm_config(mm_config, api_process_count=3)
    reserved_bytes = GiB_bytes // 4

    with patch.object(
        gpu_worker_module,
        "get_mm_gpu_memory_reservation_bytes",
        return_value=reserved_bytes,
    ) as get_reservation:
        assert worker._reserve_mm_ipc_gpu_memory(GiB_bytes) == (
            GiB_bytes - reserved_bytes
        )

    get_reservation.assert_called_once_with(mm_config, 3)
