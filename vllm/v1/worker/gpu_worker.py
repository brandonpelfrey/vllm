...
class Worker(WorkerBase):
    ...
    def _reserve_mm_ipc_gpu_memory(self, available_kv_cache_memory_bytes: int) -> int:
        ...
        The frontend (API-server) process can allocate GPU memory for
        multimodal processing. The corresponding reservation is calculated in
        the multimodal layer and subtracted here from the KV-cache budget.
        """
        mm_config = self.model_config.multimodal_config
        num_api_servers = max(1, getattr(self.parallel_config, "_api_process_count", 1))
        reserved_bytes = get_mm_gpu_memory_reservation_bytes(mm_config, num_api_servers)
        if reserved_bytes <= 0:
        ...
        logger.info_once(
            ...
            "across %d API server(s); KV cache memory reduced to %s GiB.",
